...
export default function Felhasznalo({ felhasznalo, hiba }) {
  ...
  if(hiba) return <div className="min-h-screen bg-gray-700 flex flex-col items-center">
  <title suppressHydrationWarning>{"PótVarázsló / "+felhasznalo?.nev}</title>
  <Navbar />
...
export const getServerSideProps = async (ctx) => {
  const res = await fetch(
    "http://127.0.0.1:8080/api/felhasznalo/" + ctx?.params?.felhasznaloID
  );
